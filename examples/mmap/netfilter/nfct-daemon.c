...
static int handle(struct mnl_socket *nl)
{
	...
	void *ptr;
	struct mnl_ring *ring = mnl_socket_get_ring(nl, MNL_RING_RX);

	ret = getsockopt(mnl_socket_get_fd(nl), SOL_SOCKET, SO_ERROR, &opt, &optlen);
	if (ret == 0)
		ret = opt;
	if (ret == -1) {
	...
	if (frame->nm_status == NL_MMAP_STATUS_VALID) {
		...
		fprintf(stderr, "could not get valid frame\n");
		return 0;
	}

	ret = mnl_cb_run(ptr, ret, 0, 0, data_cb, NULL);
...
int main(int argc, char *argv[])
{
	...
	/* Set netlink receiver buffer to 8 or 16 MBytes, to avoid packet drops */
	if (mnl_socket_set_ringopt(nl, MNL_RING_RX,
				   MNL_SOCKET_BUFFER_SIZE * 16, 16 * 8,
				   MNL_SOCKET_BUFFER_SIZE, 16 * 16 * 8) < 0) {
